...
class Api extends YandexObject
{
    ...
    public function confirmSenderOrders($order_id)
    {
        $this->_data = array();

        if (preg_match('/^((\d+)|((\d+\-)?MS\d+))$/', $order_id)) {
            $this->_data['order_id'] = $order_id;
            $this->_data['date_shipment'] = TIME;
        } else {
            $this->_error = YD_ERROR_WRONG_PARAM;
        ...
        return $this->request('confirmSenderOrders');
    }
    ...
    public function request($method, $force = false)
    {
        ...
        $key = $this->_data['secret_key'];

        if (!$force) {
            $this->_result = fn_get_session_data($key);
        ...
        $curl_answer = Http::post($this->api_url . $method, $this->_data);
        $this->_result = json_decode($curl_answer, true);
        fn_set_session_data($key, $this->_result, YD_CACHE_REQUEST);
