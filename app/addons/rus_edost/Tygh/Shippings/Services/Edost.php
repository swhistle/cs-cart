...
class Edost implements IService
{
    ...
    private function _getDestinationCode($destination)
    {
        ...
        if (($destination['country'] != 'ru') || ($origination != 'ru')) {
            ...
            $cities = db_get_array("SELECT c.city_code FROM ?:rus_city_descriptions as d LEFT JOIN ?:rus_cities as c ON c.city_id = d.city_id WHERE ?p AND d.lang_code = ?s", $condition, CART_LANGUAGE);

            if (count($cities) == 1) {
                $result = db_get_field("SELECT c.city_code FROM ?:rus_city_descriptions as d LEFT JOIN ?:rus_cities as c ON c.city_id = d.city_id WHERE ?p AND d.lang_code = ?s", $condition, CART_LANGUAGE);
