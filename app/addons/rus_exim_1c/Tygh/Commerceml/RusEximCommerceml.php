...
class RusEximCommerceml
{
    ...
    {
        $log = new Logs(self::$path_file);

        $log->write("Data : " . date("d-m-Y h:i:s") . " - " . $message);
    }
    ...
    {
        self::$path_file = 'exim/1C_' . date('dmY') . '/';
    ...
    public static function checkParameterFileUpload()
    {
        $message = "";

        if (self::$s_commerceml['status'] != 'A') {
        ...
        if (!empty($_SERVER['PHP_AUTH_USER'])) {
            ...
            self::$import_params['user_data'] = $user_data;
            ...
            $log_message = RusEximCommerceml::getCompanyStore($user_data);
        ...
        if (!empty($message) || !empty($log_message)) {
            ...
            self::addMessageLog($log_message);
    ...
    public static function getDirCommerceML($data_path = '')
    {
        self::$path_commerceml = fn_get_files_dir_path() . $data_path;
        ...
        if (!empty($data_path)) {
            ...
            self::$url_images = fn_get_files_dir_path() . 'from_1c/';
        }
    ...
    {
        $company_settings = Settings::instance()->getValues('rus_exim_1c', 'ADDON', false, self::$company_id);

        if (!empty($company_settings)) {
            self::$s_commerceml = $company_settings;
        }

        return self::$s_commerceml;
    }
    ...
    public static function createImportFile($filename)
    {
        ...
        $file_mode = 'w';
        list($path_commerceml, $url_commerceml, $url_images) = self::getDirCommerceML(self::$path_file);
    ...
    public static function getCountImportElements($import_data)
    {
        $count_imports = 0;
        ...
        if (!empty($import_data->{self::$cml['classifier']}->{self::$cml['properties']})) {
            $count_imports += substr_count($import_data->{self::$cml['classifier']}->{self::$cml['properties']}->asXML(), '<' . self::$cml['property'] . '>');
        }

        if (!empty($import_data -> {self::$cml['catalog']} -> {self::$cml['products']})) {
            $count_imports += substr_count($import_data -> {self::$cml['catalog']} -> {self::$cml['products']}->asXML(), '<' . self::$cml['product'] . '>');
        }

        return $count_imports;
    }

    public static function checkCountImportElements($count_imports, $import_param)
    {
    ...
    public static function initProcessImportData($import_param, $service_exchange)
    {
        $import_pos = 0;
        $pos_start = 0;
    ...
    public static function checkProcessImportData($_import, $property, $import_param, &$import_pos, $progress = false)
    {
    ...
    public static function finishImportData($progress, $import_param, $import_pos, $service_exchange, $manual)
    {
        if ($service_exchange == '') {
            if ($progress) {
    ...
    public static function importDataProductFile($import_data)
    {
        self::addMessageLog("Started import date to file import.xml, parameter service_exchange = '" . self::$import_params['service_exchange'] . "'");
        ...
        if (self::$import_params['service_exchange'] == '') {
            $count_imports = self::getCountImportElements($import_data);
            \Tygh::$app['session']['exim_1c']['f_count_imports'] = 0;

            if (self::checkCountImportElements($count_imports, 'count_imports')) {
                fn_echo("success\n");
            } else {
                fn_echo("progress\n");
                $import_file = false;
        ...
        if (empty(\Tygh::$app['session']['exim_1c']['import_products']) && empty(\Tygh::$app['session']['exim_1c']['import_features'])) {
            ...
            if ((self::$s_commerceml['exim_1c_allow_import_categories'] == 'Y') && !empty($import_data->{$cml['classifier']}->{$cml['groups']})) {
                ...
                self::importCategoriesFile($import_data->{$cml['classifier']}->{$cml['groups']}, self::$import_params);
            }
        }
        ...
        if ((self::$s_commerceml['exim_1c_allow_import_features'] == 'Y') && !empty($import_data->{$cml['classifier']}->{$cml['properties']})) {
            ...
            if (!isset(\Tygh::$app['session']['exim_1c']['import_features'])) {
                ...
                list($import_pos, $pos_start) = self::initProcessImportData('features_products', self::$import_params['service_exchange']);

                self::importFeaturesFile($import_data->{$cml['classifier']}->{$cml['properties']}, self::$import_params, $pos_start, $import_pos, $progress);

                self::checkProcessImportData($import_data->{$cml['classifier']}->{$cml['properties']}, $cml['property'], 'import_features', $import_pos, $progress);

                self::finishImportData($progress, 'features_products', $import_pos, self::$import_params['service_exchange'], self::$import_params['manual']);
            }
        ...
        if (isset(\Tygh::$app['session']['exim_1c']['import_features']) && !$progress) {
            ...
            if (isset($import_data -> {$cml['catalog']} -> {$cml['products']})) {
                ...
                self::importProductsFile($import_data -> {$cml['catalog']} -> {$cml['products']}, self::$import_params, $import_pos);
    ...
    public static function importFeaturesFile($data_features, $import_params, $data_pos_start, &$import_pos, &$progress)
    {
        ...
        if (isset($data_features -> {$cml['property']})) {
            ...
            foreach ($data_features -> {$cml['property']} as $_feature) {
                ...
                $feature_data = self::dataFeatures($feature_name, $feature_id, strval($_feature -> {$cml['type_field']}), self::$s_commerceml['exim_1c_used_brand'], self::$s_commerceml['exim_1c_property_for_manufacturer'], strval($_feature -> {$cml['id']}));
        ...
        if (self::$s_commerceml['exim_1c_used_brand'] == 'field_brand') {
            ...
            $feature_data = self::dataFeatures($cml['brand'], $feature_id, ProductFeatures::EXTENDED, self::$s_commerceml['exim_1c_used_brand'], self::$s_commerceml['exim_1c_property_for_manufacturer'], "brand1c");
    ...
    public static function dataFeatures($feature_name, $feature_id, $f_type, $used_brand, $property_for_manufacturer, $external_id)
    {
        $db = \Tygh::$app['db'];

        $feature_type = $db->getField("SELECT feature_type FROM ?:product_features WHERE external_id = ?s", $external_id);
    ...
    public static function importProductsFile($data_products, $import_params, $import_pos = 0)
    {
        ...
        list(, $product_pos_start) = self::initProcessImportData('import_products', $import_params['service_exchange']);

        $offers_pos = $import_pos;
        $progress = false;

        foreach ($data_products -> {self::$cml['product']} as $_product) {
            if ($import_params['service_exchange'] == '') {
                $offers_pos++;
            ...
            $log_message = self::addDataProductByFile($_product, self::$cml, $categories_commerceml, $import_params);
            self::addMessageLog($log_message);
        }

        self::finishImportData($progress, 'import_products', $offers_pos - $import_pos, $import_params['service_exchange'], $import_params['manual']);
    ...
    public static function startSessionUpload($param_session, $product_count)
    {
    ...
    public static function finishSessionUpload($progress, $param_session, $upload_count)
    {
        if ($progress) {
            if (!isset(\Tygh::$app['session']['exim_1c'])) {
                \Tygh::$app['session']['exim_1c'] = array();
    ...
    public static function addDataProductByFile($_product, $cml, $categories_commerceml, $import_params)
    {
        ...
        $link_type = self::$s_commerceml['exim_1c_import_type'];
        $log_message = "";
        ...
        list($guid_product, $combination_id) = self::getProductIdByFile($_product -> {$cml['id']});

        $product_data = self::getProductDataByLinkType($link_type, $_product, $cml);
        ...
        if (self::checkUploadProduct($product_id, $product_update)) {
            ...
            db_query("UPDATE ?:products SET company_id = ?i WHERE product_id = ?i", self::$company_id, $product_id);
            ...
            if ((isset($_product -> {$cml['properties_values']} -> {$cml['property_values']}) || isset($_product -> {$cml['manufacturer']})) && ($allow_import_features == 'Y') && (!empty(self::$features_commerceml))) {
                $product = self::dataProductFeatures($_product, $product, $import_params);
            }

            if (isset($_product -> {$cml['value_fields']} -> {$cml['value_field']})) {
                self::dataProductFields($_product, $product);
            }

            if (isset($_product -> {$cml['taxes_rates']}) && ($add_tax == 'Y')) {
                $product['tax_ids'] = self::addProductTaxes($_product -> {$cml['taxes_rates']}, $product_id);
            }
    ...
    public static function checkUploadProduct($product_id, $product_update)
    {
    ...
    public static function getProductIdByFile($commerceml_id)
    {
    ...
    public static function getProductDataByLinkType($link_type, $_product, $cml)
    {
        list($guid_product, $combination_id) = self::getProductIdByFile($_product -> {$cml['id']});
    ...
    public static function dataProductFile($d_product, $product_id, $external_id, $categories_commerceml, $import_params)
    {
        ...
        list($full_name, $product_code, $html_description) = self::getAdditionalDataProduct($requisites, $cml);
        ...
        $product['product'] = self::getProductNameByType($import_product_name, $d_product, $full_name, $cml);
        $product['product_code'] = self::getProductCodeByTypeCode($import_product_code, $d_product, $product_code, $cml);

        if ($import_full_description != 'not_import') {
            $product['full_description'] = self::getProductDescriptionByType($import_full_description, $d_product, $html_description, $full_name, $cml);
        }

        if ($import_short_description != 'not_import') {
            $product['short_description'] = self::getProductDescriptionByType($import_short_description, $d_product, $html_description, $full_name, $cml);
        }

        if ($import_page_title != 'not_import') {
            $product['page_title'] = self::getProductPageTitle($import_page_title, $d_product, $full_name, $cml);
        }
        ...
        if (!empty($product_id)) {
            ...
            if ($category_id == 0) {
                ...
                $category_id = db_get_field("SELECT category_id FROM ?:categories WHERE category_id = ?i AND company_id = ?i", $category_id, self::$company_id);
    ...
    public static function getAdditionalDataProduct($requisites, $cml)
    {
    ...
    public static function getProductNameByType($import_product_name, $d_product, $full_name, $cml)
    {
    ...
    public static function getProductCodeByTypeCode($import_product_code, $d_product, $_code, $cml)
    {
    ...
    public static function getProductDescriptionByType($type_description, $d_product, $html_description, $full_name, $cml)
    {
        $description = '';
    ...
    public static function getProductPageTitle($import_page_title, $d_product, $full_name, $cml)
    {
        $page_title = '';
        ...
        if (!empty($data_product -> {$cml['properties_values']} -> {$cml['property_values']})) {
            foreach ($data_product -> {$cml['properties_values']} -> {$cml['property_values']} as $_feature) {
                ...
                if (!empty($features_commerceml[$feature_id])) {
                    $product_params = self::dataShippingParams($p_feature_name, $feature_name);
    ...
    public static function dataProductFields($data_product, &$product)
    {
        $cml = self::$cml;
        ...
        if (!empty($data_product -> {$cml['value_fields']} -> {$cml['value_field']})) {
            foreach ($data_product -> {$cml['value_fields']} -> {$cml['value_field']} as $value_field) {
                ...
                if (!empty($_v_field)) {
                    $product_params = self::dataShippingParams($_v_field, $_name_field);
    ...
    public static function dataShippingParams($p_feature_name, $features_name)
    {
    ...
    public static function addProductTaxes($product_tax, $product_id)
    {
        $tax_ids = array();
        $cml = self::$cml;
        ...
        if (!empty($product_id)) {
            $tax_id = $db->getColumn("SELECT tax_ids FROM ?:products WHERE product_id = ?i AND company_id = ?i", $product_id, self::$company_id);
        ...
        $product_taxes = $db->getColumn("SELECT tax_id FROM ?:rus_exim_1c_taxes WHERE tax_1c = ?s AND company_id = ?i", strval($product_tax -> {$cml['tax_rate']} -> {$cml['rate_t']}), self::$company_id);
    ...
    public static function addProductCombinationsNewSchema($combinations, $product_id, $import_params, $combination_id = 0, $variant_data = array(), $article_option = "")
    {
        ...
        if (empty($combination_id)) {
            ...
            foreach ($combinations as $_combination) {
                ...
                if (!empty($data_variant['variant_id'])) {
                    ...
                    self::addNewCombination($product_id, strval($_combination -> {$cml['id']}), $add_options_combination, $import_params, $article_option);
                }
            }
            ...
            if (!empty($add_options_combination)) {
                self::addNewCombination($product_id, $combination_id, $add_options_combination, $import_params, $amount, $article_option);
            }
        }
    }
    ...
    public static function addNewCombination($product_id, $combination_id, $add_options_combination, $import_params, $amount = 0, $article_option = "")
    {
        ...
        $o_article = db_get_field("SELECT product_code FROM ?:product_options_inventory WHERE external_id = ?s", $combination_id);
    ...
    public static function importProductOffersFile($data_offers, $import_params)
    {
        ...
        $all_currencies = self::dataProductCurrencies();
        ...
        if (isset($data_offers -> {$cml['prices_types']} -> {$cml['price_type']})) {
            ...
            if ($create_prices == 'Y') {
                $data_prices = db_get_array("SELECT price_1c, type, usergroup_id FROM ?:rus_exim_1c_prices WHERE company_id = ?i", self::$company_id);
        ...
        foreach ($data_offers -> {$cml['offers']} -> {$cml['offer']} as $offer) {
            ...
            if (isset($offer -> {$cml['prices']}) && !empty($price_offers)) {
                ...
                $product_prices = self::conversionProductPrices($offer -> {$cml['prices']} -> {$cml['price']}, $_price_offers);
    ...
    public static function exportAllProductsToOrders($lang_code)
    {
        $params = array(
            'company_id' => self::$company_id,
            'company_name' => true
        ...
        $order_data = array_merge($order_data, self::$import_params['user_data']);
    ...
    public static function dataOrderToFile($xml, $order_data, $lang_code)
    {
        ...
        $order_xml = self::getOrderDataForXml($order_data, $cml);

        $order_xml[$cml['contractors']][$cml['contractor']] = self::getDataOrderUser($order_data);
        ...
        $order_xml[$cml['products']] = self::dataOrderProducts($xml, $order_data, $rate_discounts);
        ...
        list($payment, $shipping) = self::getAdditionalOrderData($order_data);
    ...
    public static function getOrderDataForXml($order_data, $cml)
    {
    ...
    public static function getAdditionalOrderData($order_data)
    {
    ...
    public static function getDataOrderUser($order_data)
    {
        $cml = self::$cml;
     ...
     * @return void
    */
    public static function setDataProductByOptions($product_id, $product_options, &$external_id, &$product_name)
    {
        $combinations = '';
        ...
        $options_inventory = db_get_row("SELECT external_id, combination_hash FROM ?:product_options_inventory WHERE product_id = ?i AND combination = ?s", $product_id, $combinations);
        ...
        if (empty($options_inventory) && !empty($options_ids)) {
            ...
            $options_inventory = db_get_row("SELECT external_id, combination_hash FROM ?:product_options_inventory WHERE product_id = ?i AND combination_hash = ?s", $product_id, $combination_hash);
        }
    ...
    public static function dataOrderProducts($xml, $order_data, $discount = 0)
    {
        $cml = self::$cml;
        ...
        foreach ($order_data['products'] as $product) {
            ...
            if (!empty($product['product_options']) && $export_options == 'Y') {
                self::setDataProductByOptions($product['product_id'], $product['product_options'], $external_id, $product_name);
            }
    ...
    public static function importFileOrders($xml, $lang_code)
    {
        ...
        if (isset($xml->{$cml['document']})) {
            ...
            foreach ($orders_data as $order_data) {
                ...
                $_order_id = db_get_field("SELECT order_id FROM ?:orders WHERE order_id = ?i", $import_id);
                if (!empty($_order_id)) {
                    $order_id = $_order_id;
                }
    ...
    public static function dataOrderTaxs($taxes)
    {
        $data_taxes = array();
        $products_tax = array();
        $commerceml_tax = db_get_hash_array("SELECT * FROM ?:rus_exim_1c_taxes WHERE company_id = ?i", 'tax_id', self::$company_id);
        if (!empty($taxes)) {
    ...
    {
        $cml = self::$cml;
        $data_prices = db_get_array("SELECT price_1c, usergroup_id, type FROM ?:rus_exim_1c_prices WHERE company_id = ?i", self::$company_id);
        if (empty($data_prices)) {
            $data_prices = db_get_array("SELECT price_1c, type, usergroup_id FROM ?:rus_exim_1c_prices");
        }
