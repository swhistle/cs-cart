...
if ($mode == 'autocomplete_city') {
    ...
    if (defined('AJAX_REQUEST')) {
        ...
        if (!empty($data_states) && !empty($params['q'])) {
            ...
            $cities = db_get_array("SELECT d.city, c.city_code FROM ?:rus_city_descriptions as d ?p WHERE city LIKE ?l AND lang_code = ?s  ?p  LIMIT ?i", $join , $search , CART_LANGUAGE, $condition, 10);
