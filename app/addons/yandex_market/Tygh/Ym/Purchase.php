    ...
    public function cart($items, $delivery)
    {
        ...
        if (!empty($group['shippings'])) {
            ...
            foreach ($group['shippings'] as $shipping) {
                if (empty($shipping['yml_shipping_type'])) {
                    continue;
                }

                if (!empty($shipping['yml_order_before'])) {
                    $shipping_plus_day = ($current_hour > $shipping['yml_order_before']) ? SECONDS_IN_DAY : 0;
                }
                ...
                $_shipping = array(
                    ...
                    'price' => $shipping['rate'],
                    'dates' => array(
                        'fromDate' => date('d-m-Y', TIME + $shipping_plus_day + $shipping['yml_from_date'] * SECONDS_IN_DAY),
                    ),
                );
