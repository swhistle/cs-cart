        ...
        if (!empty($this->_shipping_info['package_info']['packages'])) {
            ...
            if ($packages_count > 0) {
                foreach ($packages as $id => $package) {
                    ...
                    if (!empty($_REQUEST['order_id'])) {
                        ...
                        foreach ($package['products'] as $cart_id => $amount) {
                            ...
                            if (!empty($extra['product_options_value'])) {
                                ...
                                $p_weight = fn_apply_options_modifiers($product_options, $p_weight, 'W');
                                if ($p_weight == 0) {
                                    $p_weight = round(100 / Registry::get('settings.General.weight_symbol_grams'), 3);
                                }
                    ...
                    $weight = round($weight * Registry::get('settings.General.weight_symbol_grams') / 1000, 3);
