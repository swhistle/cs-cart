                    ...
                    if (!empty($order[1])) {
                        foreach ($d_status as $state) {
                            $data_status[$order[0] . '_' . $order[1] . '_' . $state['@attributes']['Code']] = array(
                                ...
                                'city_name' => db_get_field("SELECT a.city FROM ?:rus_city_descriptions as a LEFT JOIN ?:rus_cities as b ON a.city_id=b.city_id WHERE b.sdek_city_code = ?s", $state['@attributes']['CityCode']),
