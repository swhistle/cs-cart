...
function fn_get_products($params, $items_per_page = 0, $lang_code = CART_LANGUAGE)
{
    ...
    if (isset($params['q']) && fn_string_not_empty($params['q'])) {
        ...
        $search_text_fields_table_alias = 'descr1';

        /**
         * Executed before any search SQL conditions are generated.
         *
         * @param array  $params                         List of parameters passed to fn_get_products functions
         * @param int    $items_per_page                 Number of products to fetch per each page
         * @param string $lang_code                      Language code
         * @param string $search_text_fields_table_alias Alias of a product descriptions table to use at search conditions SQL. Defaults to "descr1", which is the "product_descriptions" table.
         * @param array  $fields                         List of fields for retrieving
         * @param array  $sortings                       Sorting fields
         * @param string $condition                      String containing SQL-query condition possibly prepended with a logical operator (AND or OR)
         * @param string $join                           String with the complete JOIN information (JOIN type, tables and fields) for an SQL-query
        ...
        fn_set_hook(
            ...
            $items_per_page,
            $lang_code,
            $search_text_fields_table_alias,
            $fields,
            $sortings,
            $condition,
            $join,
        ...
        foreach ($query_pieces as $piece) {
            ...
            $tmp = db_quote("({$search_text_fields_table_alias}.search_words LIKE ?l)", '%' . $piece . '%'); // check search words
            if ($params['pname'] == 'Y') {
                $tmp .= db_quote(" OR {$search_text_fields_table_alias}.product LIKE ?l", '%' . $piece . '%');
            }
            if ($params['pshort'] == 'Y') {
                $tmp .= db_quote(" OR {$search_text_fields_table_alias}.short_description LIKE ?l", '%' . $piece . '%');
                $tmp .= db_quote(" OR {$search_text_fields_table_alias}.short_description LIKE ?l", '%' . htmlentities($piece, ENT_QUOTES, 'UTF-8') . '%');
            }
            if ($params['pfull'] == 'Y') {
                $tmp .= db_quote(" OR {$search_text_fields_table_alias}.full_description LIKE ?l", '%' . $piece . '%');
                $tmp .= db_quote(" OR {$search_text_fields_table_alias}.full_description LIKE ?l", '%' . htmlentities($piece, ENT_QUOTES, 'UTF-8') . '%');
            }
            if ($params['pkeywords'] == 'Y') {
                $tmp .= db_quote(" OR ({$search_text_fields_table_alias}.meta_keywords LIKE ?l OR {$search_text_fields_table_alias}.meta_description LIKE ?l)", '%' . $piece . '%', '%' . $piece . '%');
            }
